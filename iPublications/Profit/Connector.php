...
abstract class Connector
{
    ...
    public function __construct(Connection $P_o_ConnectionSettings)
    {
        ...
        if ($P_o_ConnectionSettings->HasSoapCallUsername()) {
            $this->SetUserId($P_o_ConnectionSettings->GetSoapCallUsername());
        }
        if ($P_o_ConnectionSettings->HasSoapCallPassword()) {
            $this->SetPassword($P_o_ConnectionSettings->GetSoapCallPassword());
        }
        if ($P_o_ConnectionSettings->HasSoapCallEnvironment()) {
            $this->SetEnvironmentId($P_o_ConnectionSettings->GetSoapCallEnvironment());
        }
        if ($P_o_ConnectionSettings->HasSoapCallToken()) {
            $this->SetToken($P_o_ConnectionSettings->GetSoapCallToken());
        }
    }

    final public function setConnectorNameIsSet($L_s_connectorName = '')
    {
        if (!empty($L_s_connectorName)) {
            $this->M_b_connectorNameSet = true;
        }
    }
    ...
    {
        if (isset($this->M_s_Element_userId)) {
            unset($this->M_s_Element_userId);
        }
        if (isset($this->M_s_Element_logonAs)) {
            unset($this->M_s_Element_logonAs);
        }
        if (isset($this->M_s_Element_password)) {
            unset($this->M_s_Element_password);
        }
    }
