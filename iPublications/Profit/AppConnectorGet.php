 ...
 *        1003 = Sorting / Limit shoud bel set BEFORE call!
 *        1004 = No Token given or invalid
 */
class AppConnectorGet extends Connector
{
    private $M_a_fields;
    private $M_a_results;
    private $M_i_limit;
    ...
    final public function __construct(Connection $P_o_ConnectionSettings)
    {
        ...
        $this->SetRequiredElement(Connector::SKIP);
        $this->SetRequiredElement(Connector::TAKE);
    ...
    final public function Execute($P_s_ConnectorName = false)
    {
        ...
        if ($P_s_ConnectorName !== false) {
            $this->SetConnectorId($P_s_ConnectorName);
        }

        if (!preg_match("@[A-Z0-9a-z]{32,}@", $this->GetToken())) {
            throw new Exception("No Token given or invalid.", 1004);
        }
    ...
    final public function SetLimit($P_i_count = 1)
    {
        if (!isset($this->M_a_results)) {
            ...
            $this->SetTake($this->M_i_limit);
        } else {
            throw new Exception("Limit should be set BEFORE call!", 1003);
        }
    }
